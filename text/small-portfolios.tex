...
\supplementdetails[subcategory={Full experimental data (review link, data will be moved to a public repository after review)}]{Dataset}{https://bwsyncandshare.kit.edu/s/yKtJ34KTyqBtcJn}
...
One can combine a solver portfolio with a perfect oracle that then selects the fastest solver from the portfolio for each instance in the benchmark set. 
...
Moreover, the construction of a prediction-based solver-selection process is complex and quite time-consuming, in particular, preparing the necessary training data.
...
In particular, this includes choosing a type of prediction model as well as instance features used to learn the model. 
...
We begin with portfolios in general and continue with instance-specific solver selection, including approaches to automatically configure and analyze such techniques. 
...
This is the expected result for a benchmark set that contains similar amounts of SAT and UNSAT instances. 
...
They solve the \emph{K-Portfolio Problem} optimally with an Satisfiability Module Theories (SMT) solver and use \emph{greedy search} to generate approximate solutions.
...
In reality, one does not have access to an oracle, but one may train a prediction model $m : I \rightarrow P$ for a solver portfolio $P$.
...
Clearly, the VBS-based portfolio cost $c_{T}(P)$ is a lower bound for the actual cost of a model-based portfolio $P$.
...
This is because adding further solvers only increases the number of choices for the oracle, which always chooses optimally.
...
This is a baseline in the study of Nof and Strichman~\cite{nof2020real}. 
...
\begin{itemize}
	...
	\item \emph{K-best}:
	We use portfolios out of the $k$ best individual solvers as a simple baseline.
...
As a last example, several features base on probing, i.e., running solvers for a short time and quantifying their progress over that period.

Some feature values are missing, since the feature extractor exceeded time- or memory limits.
For predictions, we replace missing values with a constant value that is out of the range of the features.
...
In this section, we focus on portfolios as a whole, in particular, the portfolio performance $c_T(P)$.
...
In contrast, the theoretical, submodularity-based \emph{upper bound} for \emph{greedy search} clearly is higher than the actual portfolio cost and thus, too loose to serve as a good estimate.
...
In particular, there is a large variance in the PAR-2 score of portfolios in the beam for small $k$, and this variance becomes smaller in later iterations, i.e., the top $w$ portfolios become more similar in performance.
...
We have carried out a correlation analysis on the 1000 portfolios that result from \emph{random sampling} for a certain value of $k$. 
...
This could cause a bad \mbox{PAR-2} value for model-based portfolios, which we will analyze next.
...
The plot compares the PAR-2 score of solvers recommended by the prediction model to two competitors without prediction model, computed on the same portfolios.
...
The single best solver (SBS) of each portfolio serves as a baseline, corresponding to always recommending one particular solver.
...
Averaging importance over all trained random-forest models, the most important feature has an importance of 1.66\%, the least important one an importance of 0.01\%.
...
Also, the low prediction performance indicates that the features from SATzilla~2012 are not sufficiently discriminating to reliably recommend solvers on our datasets.
...
New SAT solvers that improve performance only on a few benchmark instances can be assets to solver portfolios nevertheless. 
...
Efficient implementations of feature extractors and studying feature importance on datasets that represent the state-of-the-art are subject to future work. 
