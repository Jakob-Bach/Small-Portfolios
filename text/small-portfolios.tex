...
\supplementdetails[subcategory={Full experimental data (review link, data will be moved to public repo after review)}]{Dataset}{https://bwsyncandshare.kit.edu/s/yKtJ34KTyqBtcJn}
...
%Note that the term \emph{portfolio} is used in different contexts in literature~\cite{kerschke2019automated}.
%For examples, one can also run multiple algorithms from a portfolio in parallel.
%These algorithms might even exchange information, e.g., share clauses learned from the SAT instance.
%In our work, we only consider portfolios from which exactly one solver is selected per instance.
