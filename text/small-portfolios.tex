...
Solvers are usually evaluated based on compilations of benchmark instances which represent \emph{diverse} interesting application scenarios for SAT solvers. 
...
Another observation is an recurrent emergence of new heuristics and methods which contribute to the performance on only a narrow subset of instances. 
Such approaches can be seen as complementary to solvers with the best average behavior.
In order to highlight such insular progress, one can evaluate solvers separatly only on specific types instances.
One example for this kind of evaluations is the introduction of an application track in SAT Competitions in which the participating solvers are additionally evaluated on a large benchmark set stemming purely from one specific application. 

\begin{example}[Crypto Track in SAT Competition 2021]
The SAT Solver \emph{SLIME} was not competitive in the evaluations on the diverse set of instances in the Main Track of SAT Competition~2021. 
However, \emph{SLIME} achieved the best score in the Crypto Track where SAT solvers were evaluated on a set of $200$ instances purely stemming from cryptographic applications. 
\end{example}

Another possibility to capture the progress in SAT solving beyond measuring each solver's average performance is to consider portfolios, which are sets of solvers.  
A solver portfolio can be combined with a perfect oracle which selects the fastest solver for each instance in the benchmark set. 
Such a theoretical construction is commonly referred to as the \emph{Virtual Best Solver} (VBS). 
One can determine an alternative ranking of solvers based on their marginal contribution to the VBS, which has already been suggested in~\cite{Xu:2012:EvalContribVBS}. 

\begin{example}[Special Price in SAT Competition 2021]
In SAT Competition 2021, the performance of the VBS of all possible portfolios of size two which can be drawn from the entirety of participating solvers has been evaluated. 
The best performing tuple of solvers (in terms of their VBS), contains the solver \emph{CaDiCaL\_PriPro}, which was not competitive in the overall evaluation and thus given the new \emph{Special Innovation Price}.
\end{example}

Besides the theoretical performance gains of the VBS, it has been shown that combining a portfolio with a complex automatic solver selection process which also involves runtime prediction models can considerably outperform single best solvers~\cite{xu2008satzilla}. 
However, such complex constructions usually lack to capture the state-of-the-art in SAT solving as the integration of new SAT solvers is time consuming. 

% Problem Statement
First, one has to decide which and how many solvers to combine in a portfolio. 
...
The \emph{$K$-Portfolio Problem}, which is to determine an optimal $k$-portfolio, is an NP-hard optimization problem~\cite{nof2020real}. 

Second, the assembly of a dataset which contains instance features as well as runtime data, the training of instance classifiers and runtime prediction models requires a considerable amount of computation time. 


\paragraph{Contributions}

In the presented study, we \emph{capture and evaluate} the state-of-the-art in SAT solving in terms of portfolio performance based on very recent datasets which consist of the runtime data of solvers participating in SAT~Competitions~2020 and~2021. 

We systematically construct $k$-portfolios and analyze the impact of the portfolio size $k$ on portfolio performance. 
For exact search, we encode the K-Portfolio Problem as an \emph{integer optimization problem}. 
We also analyze the quality of several approximate and random solutions. 
With the presented approaches, we provide means to \emph{create optimal portfolios} in an ad-hoc manner. 

For each portfolio size $k$, we also train runtime prediction models which use random forests trained with the instance feature records of SATzilla~2012~\cite{xu2012satzilla2012}. 
We analyze the prediction performance and the runtimes of the resulting automatic solver selection approaches with respect to $k$. 
We make all our code and data available online.


\paragraph{Results}

Naturally, portfolio performance improves most when adding the first few solvers returns diminish with increasing $k$. 
The best approximate solutions to the $K$-Portfolio Problem could be found by \emph{beam search}. 
They have very similar performance to the optimal solution, even for a minimal beam width of one. 

Our prediction models exhibit a rather low \emph{prediction performance} in our experiments. We hypothesize that this is due to the instance features available in our dataset. 
The \emph{actual runtime} of portfolios with predictive models is not even close to the theoretical improvement we could have with a perfect oracle. 
