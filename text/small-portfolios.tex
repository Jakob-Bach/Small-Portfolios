...
It has lots of practical applications, e.g., verification of hardware and software~\cite{Kaufmann:2021:Amulet,Buning:2020:QPRVerify}, product configuration~\cite{Janota:2014:Configuration}, cryptanalysis~\cite{Nejati:2020:CDCLCrypto}, or planning~\cite{Schreiber:2021:Lilotane}.
Today, SAT solvers are not only used to solve hard combinatorial problems in industry, but also previously open problems in mathematics~\cite{Heule:2016:Pyth,Heule:2018:Schur}. 

One can observe continuous progress in SAT solving methods, heuristics, and their implementations in SAT solvers. 
Solvers are usually evaluated based on compilations of benchmark instances which represent diverse interesting application scenarios for SAT solvers. 
...
Another observation is an ongoing emergence of new heuristics which contribute to the performance on narrow subsets of instances. 
Such approaches are complementary to solvers with the best average behavior.
In order to highlight such insular progress, one can evaluate solvers on projections of the instance set, e.g., projections to a certain family of instances.
Another possibility to capture this kind of progress %KB: Es liest sich, als wären beide Alternativen gleich gut. Die erste ist aber nur reißerisch, die zweite ist seriöser. Vermutlich sehen Sie das auch so. Das sollte explizit gesagt werden.
is to consider the marginal contributions of solvers to the performance of a solver portfolio. %KB: Hier ist die Reihenfolge komisch. Stattdessen: "Another possibility is relate to so-called portfolios, which are ... (Vielleicht sollten auch die folgenden Einlassungen zu Portfolios ("It has been shown that ...") hier gleich kommen. bzw. die Erklärungen von Portfolios gesamthaft vor dem 'emergence'-Text.) One cann now quantify the improvement coming from a new solver as ..." 
Portfolios are sets of solvers, from which one solver is chosen for each SAT instance. %KB: Vielleicht noch zu kurz. Wer choost? (Oder ist hier alles erlaubt?) Wann wird gechoost? 

It has been shown that combining several SAT solvers following a complex solver selection process which also involves runtime prediction models can considerably outperform single best solvers~\cite{xu2008satzilla}. %KB: Hier dann aber gleich sagen, was die Limitierungen beispielsweise dieser Studie sind, nicht es erst weiter unten andeuten.
%KB: Selbst wenn sie schneller sind, ist aber der Rechenaufwand vermutlich deutlich höher, stimmt das? Wenn ja, würde ich das hier explizit sagen, auch um das, was folgt, gut zu motivieren.
Such complex portfolios usually lack of explainability of their performance gains. %KB: Was ist der Unterschied zwischen 'portfolio' und 'complex portfolio'? -- Explainability kommt hier unmotiviert -- warum will man es hier?
Moreover, they compete with the ongoing and considerable performance gains through research on singular solvers. %KB: Den Satz verstehe ich nicht.

\paragraph{Problem Statement}

SAT solving is a very active area of research such that complex handcrafted portfolios can be quickly outdated %KB: Das hier gehört nach vorn, zu dem 'Emergence'-Text. hier nicht noch einmal ansetzen bitte.
and lack to capture the state-of-the-art. 
We want to find out to which extent portfolios with just a few state-of-the-art solvers are sufficient to improve solving performance. 
...
Determination of the optimal, i.e., best performing, portfolio of size $k$ in a given set of $n$ solvers is an NP-hard optimization problem such that exhaustive search quickly becomes infeasible. %KB: Der Text ab 'such that ...' scheint mir entbehrlich zu sein, ist ja NP-hard.
So we focus on the quality of approximate solutions to that problem. 

A good performance of such $k$-portfolios does not necessarily imply a good performance in combination with a runtime prediction model. %KB: runtime prediction im aktuellen Kontext sollte weiter oben explizit motiviert werden.
With increasing $k$, solvers in portfolios are much harder to discriminate. %KB: Den Satz verstehe ich nicht.
...
\texttt{SC2020} consists of runtime data for 48~solvers and 400~instances from the SAT~Competition~2020, \texttt{SC2021} of runtime data for 46~solvers and 400~instances from the SAT~Competition~2021. 
In our experiments, we look at all possible portfolio sizes $k$.
For approximate determination of optimal portfolios of size $k$, we apply \emph{beam search} with varying beam widths.
We compare the quality of such approximations to the results of an exact search. %KB: Das wurde bereits gesagt.
For the exact search, we encode the \emph{K-Portfolio Problem} as an integer optimization problem. 

Exact solutions to the \emph{K-Portfolio Problem} are optimal in the sense that if one always picks the fastest solver in the optimal $k$-portfolio for each instance, %KB: Sind 'optimal solution' und 'optimal portfolio' hier dasselbe? Ich vermute, die Formulierung ist umständlich. Warum nicht einfach in etwa "A k-portfolio is optimal if it outperforms ..."?
it outperforms all other $k$-portfolios that can be drawn from the given $n$ solvers. %KB: Das gehört nach vorne. Dorthin, wo zum ersten mal vom 'optimal behavior' oder so die Rede ist.
For the evaluation of our approach, %KB: Sorry, aber was ist 'our approach'? MMn wurde bis jetzt nichts Neues vorgeschlagen -- es geht um Evaluierung?!
for all $k$, we consider both the theoretically optimal performance of portfolios %KB: Welche portfolios? Ich denke leider, dass es immer noch unklar ist.
as well as the performance if a prediction model makes instance-specific solver recommendations.
We compare to two baselines, one that randomly samples $k$ solvers, and another one that selects the $k$ individually fastest solvers. --
We make all our code and data available online.

\paragraph{Results}

For both datasets, we see that adding solvers to a portfolio has diminishing returns, i.e., portfolio performance improves most when adding the first few solvers.
...
We also observe that the prediction models exhibit a rather low \emph{prediction performance} in our experiments. We hypothesize that this is due to the instance features available in our dataset.
...
The marginal returns through adding solvers are stagnating quickly, %KB: Das wurde doch gerade gesagt -- jetzt nicht noch einmal ansetzen. (Bitte beides zusammenführen.)
such that beyond portfolio sizes of $k=3$ in \texttt{SC2020} ($k=4$ in \texttt{SC2021}), the runtimes of portfolios with predictive models does not improve further. %KB: Ich würde hier noch einen Satz hinzufügen, dass das ja schon ein wichtiges Ergebnis ist, weil man also sehr früh (früher als von uns erwartet) mit der Konstruktion von Portfolios aufhören kann (oder so ähnlich).
...
This was fundamentally different in SAT~Competition~2021, where the same organizers gave a \emph{special innovation price} to a solver which was not competitive in the standalone evaluation but part of the best performing $2$-portfolio (drawn from the solvers in that particular competition). %KB: Wieso ist das fundamentally differet. Sie hätten ja schon im Jahr davor erzwingen können, dass ein nicht competitive solver dabei sein muss?!? 
...
Another point-of-view on solver complementarity is given by instance-specific solver selection approaches. %KB: Ich würde das aktiv formulieren. %KB: Es ist unklar, wie solver selection sonst funktioniert.
...
Overall, portfolio approaches can become quite complex, and it is unclear which parts of the portfolio solution are really necessary to achieve a good performance %KB: Was ist eine portfolio solution?
...
For example, we systematically analyze parametrization of \emph{beam search}. %KB: Sollte in der Einleitung auch gesagt werden, denke ich.
Further, we compare against random portfolios of size $k$ as a baseline.
Finally, we solve the \emph{K-Portfolio Problem} exactly, %KB: Weiter vorn hieß es, auch der Fokus dieses Artikels sei auf approximate solutions?!?
...
While we build on the theoretical results of Nof and Strichman, our evaluation is different. %KB: Nur 'different' oder auch 'besser'? Letzteres wäre gut, ich sehe es allerdings nicht wirklich. 
...
We further evaluate the runtimes of an instance-specific algorithm selector which is trained for the such determined $k$-portfolios. %KB: 'the such determined' ist unklar (und auch schlechtes Englisch).
