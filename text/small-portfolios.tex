...
One result of ours is that the portfolios found by our approximate approach are just as good as the optimal solution in practice. 
...
Combining a portfolio with a solver-election process which includes runtime predictions usually outperforms individual best solvers by much~\cite{xu2008satzilla}. 
But the relative performance gains of such approaches on instances in the very diverse \emph{application} category are usually much smaller compared to the relative gains obtained in the categories of \emph{crafted} instances or \emph{random} instances~\cite{Xu:2012:EvalContribVBS,Collautti:2013:SNNAP}.
