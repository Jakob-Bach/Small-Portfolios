...
\paragraph{Motivation} %KB: DAs Layout mit a), b) usw. zur Gliederung der Einleitung ist mMn sehr unüblich, nicht? %MI: ist wohl so vorgegeben, könnten zu subsections wechseln, was aber platz kostet (->hartes page-limit von 8 seiten incl. refs)
...
%We train such prediction models for all portfolios found by our solution approaches described earlier. %KB: "for" --Y "with", ist das gemeint? %MI: eher, dass jedes k-portfolio sein eigenes vorhersagemodell bekommt
...
%Random forests 
which are also used in the well-known portfolio approach SATzilla2012~\cite{xu2012satzilla2012}. 

To analyze if prediction performance improves due to ensembling, we train models with one, ten, and 100 trees.
The prediction approach in SATzilla2012 has two differences compared to ours:
...
%MI: Instanz-Klassifikation ist noch ein wichtiges Merkmal von SATzilla!
