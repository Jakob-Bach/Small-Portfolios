...
Our prediction models exhibit a rather low \emph{prediction performance} in our experiments. 
We hypothesize that this is due to the instance features available in our dataset. 
Also the \emph{actual runtime} of portfolios with predictive models is not even close to the theoretical improvement we could have with a perfect oracle. 

Still, we can observe considerably performance improvements with runtime prediction for $k$-portfolios up to portfolios of size $k = 4$. 
For larger $k$, the runtimes of portfolios with predictive models does not improve further. 
This shows that it does not make sense to combine very many solvers to a portfolio. 
