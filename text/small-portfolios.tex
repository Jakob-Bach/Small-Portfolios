...
%TODO change title, as we do no only analyze optimal portfolios; mabye "Analyzing Small Portfolios of SAT Solvers"
%TODO might even drop "small" in favor of the more technical "k-" (if we did that, we needed to adapt the remaining text, which consistently uses "small portfolio" at the moment)
...
% JB: I would remove the previous two sentences (as it was before 219d873fb2), as they don't really fit into the portfolio story and there is quite a gap to the following paragraph
...
%TODO describe "SAT Competition 2020" paper (if published), which does a lot of analysis on single solvers and some analysis on portfolios; citation already exists: \cite{SC2020:AIJ}
%TODO some references have very long booktitles at the moment; if space is neeed, can use short conference/workshop name instead
...
Additionally, there is a bound on the costs of a portfolio found by greedy search.
...
\begin{equation}
	u_{T}(P_{greedy}^k) \geq (1 - \frac{1}{e}) * \max_{|P| \leq k}{u_{T}(P)}
\end{equation}

I.e., there is a lower bound on the utility of a portfolio found by greedy search.
With a minor transformation, this is an upper bound on the cost of a portfolio found by greedy search:

\begin{equation}
	c_{T}(P_{greedy}^k) \leq (1 - \frac{1}{e}) * \min_{|P| \leq k}{c_{T}(P)} + \frac{1}{e} * c_W
...
%TODO @Markus vielleicht hast du noch eine bessere Referenz für gate recognition, ich habe erst mal deine Diss genommen
...
\begin{figure*}[t]
	...
	\caption{Comparison of different solution approaches for objective \emph{PAR2}.}
...
\begin{figure}[t]
	...
	\caption{Test-set MCC for random-search portfolios. Random forests with 100 trees.}
...
\begin{figure*}[t]
	...
	\caption{Objective value for beam-search-portfolios with $w=100$. Random forests with 100 trees.}
