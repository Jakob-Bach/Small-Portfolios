...
%JB: to save space, I would remove the following sentence and example, and directly go to portfolios (finding + evaluating specialized solvers is not a topic of this article; portfolios are the only topic, and not just "Another possibility" for something)
...
%JB: might be a bit misleading, one could think we predict what to put into a portfolio, instead of predicting which solver to use from a given portfolio
...
%JB: unfair, as far as I know, k-best was only used in the study of Nof and Strichman, and they only included it as a baseline to greedy search
According to our experiments however, relying on an off-the-shelf prediction model has been a mixed bag:
%JB: transition a bit strange, there is no direct contrast ("however"), as we simpy switch to describing results from the 2nd part of our experiments
...
%JB: "complex hybridization" is both a complicated and a vague term
...
%JB: what is the instance classification?
...
In addition to including more search algorithms, we also analyze portfolios with prediction models.
...
In preliminary experiments, we also tried two adaptations of the prediction approach, which are part of SATzilla2012:
First, one can train a classifier for each pair of solvers in the portfolio instead of training one classifier that makes a multi-class prediction.
...
For predictions, we make use of 143 features to characterize instances, of which $138$ features are from the feature extractor of SATzilla~2012~\cite{xu2008satzilla, xu2012satzilla2012} and five features are from a gate-recognition approach~\cite{Iser:2015:GateRecognition}. 
...
\todo{Update to imputation with out-of-range value (probably small impact)}
...
If we report numbers for the solution approaches in the following, we refer to the training dataset, as the solution approaches conduct their search on the training set.
...
We also evaluate the performance of these portfolios with prediction model, i.e., the PAR-2 score of the predicted solvers.
