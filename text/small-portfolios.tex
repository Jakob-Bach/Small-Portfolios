...
\author{
	...
	\IEEEauthorblockA{
		\textit{KIT Department of Informatics} \\
...
We also investigate how effective off-the-shelf prediction models are for instance-specific solver recommendations. %KB: Replace 'instance-specific' with 'problem-specific', or is this clear?
One result is that the portfolios found with an approximate approach proposed here is as good as the optimal solution in practice. 
...
One way to leverage complementarity of solvers is with portfolios.
...
However, the relative performance gains of such approaches on instances in the very diverse \emph{application} category are usually much smaller than the gains in the categories of \emph{crafted} or \emph{random} instances~\cite{Xu:2012:EvalContribVBS,Collautti:2013:SNNAP}. %KB: Wie kann es nicht besonders gut sein in der Kategorie 'very diverse applications', wenn es in der Kategorie 'random' gut ist? Ist das nicht im Grunde das gleiche? 
In some cases, they are even evaluated on random instances only~\cite{Kadioglu:2010:ISAC}. %KB: Ich glaube, das trifft es nicht ganz. Ihr Punkt war doch, dass eine Evaluierung über random Instanzen, die man manchmal antrifft, die Ansätze leistungsfähiger erscheinen lassen, als sie in der wirklichkeit dann sind?!? Vielleicht: "We argue that evaluations confined to random instances, such as the one in \cite{Kadioglu:2010:ISAC}, let the solvers appear in a better light than ones covering various domains." Oder so ähnlich.
Moreover, such constructions %KB: Unklarer Bezug: Welche Konstruktionen? Hier vielleicht: "such constructions (prediction-based solver selection) are complex ..."?
are complex and quite time-consuming. In consequence, more often than not, comparative studies in SAT solving do not include implementations applying this principle to state-of-the-art solvers and recent benchmarks of interest.
...
Second, the specifics of a solver-selection process based on performance predictions currently are unclear. 
...
For portfolios of size 5 and larger, the marginal gain in VBS performance decreases to less than 100 seconds by adding more solvers.  
...
% Figure belongs to "Evaluation" ("General Trend"), but placed here for better distribution of figures in paper
\begin{figure*}[t]
	\centering
...
% Figure belongs to "Portfolio Performance", but placed here for better distribution of figures in paper
\begin{figure*}[t]
	\centering
...
It has addressed the important question of how large portfolios should actually be. There has been a strong improvement in the objective value for small portfolios, but a small impact once a certain size is reached. 
...
However, these models have not performed satisfactorily on the given datasets: The objective value of portfolios with predictions did not continuously improve with portfolio size.
Note that relative performance gains through $k$-portfolios are usually much lower on application instances~\cite{Collautti:2013:SNNAP}. %KB: Den letzten Satz habe ich nicht verstanden. Gehört er in die Conclusions? Er klingt wie eine technische Erläuterung, der in einer Zusammenfassung des Artikels gesamthaft nicht unbedingt erscheinen muss.

We see room for improved prediction performance, by adapting the set of instance features in particular,
for instance by using features describing the community structure of graph representations of SAT instances.  
