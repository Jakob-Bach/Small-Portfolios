...
But the relative perforance gains of such approaches on instances in the very diverse \emph{application} category are usually much smaller compared to the relative gains obtained in the categories of \emph{crafted} instances or \emph{random} instances~\cite{Xu:2012:EvalContribVBS,Collautti:2013:SNNAP}.
In some cases they are even evaluated on random instances only~\cite{Kadioglu:2010:ISAC}.
Moreover, such constructions are complex and quite time-consuming, such that, more often than not, comparative studies in SAT solving do not include implementations applying this principle to state-of-the-art solvers and recent benchmarks of interest.
...
For exact search, we encode the \emph{K-Portfolio Problem} as an integer optimization problem (which was faster than alternative approaches, according to preliminary experiments of ours). 
...
%JB: I don't fully understand this and it's kind of vague (not clear what "ad hoc" means in this context)
...
%JB: what is the instance classification? %MI: 6 Klassen = 3 Kategorien x {SAT, UNSAT}
...
They evaluate their approach on anytime search algorithms for an allocation problem, focusing on solution quality after a timeout of one second. 
...
Note that relative performance gains through $k$-portfolios are usually much lower on application instances~\cite{Collautti:2013:SNNAP}. 

There is room for improvement regarding prediction performance, in particular, by adapting the set of instance features,
e.g., using features describing the community structure of graph representations of SAT instances.  
Such new kinds of features have correlated well with solver performance on application instances in recent studies~\cite{Ansotegui:2017:StructureFeatures,Li:2021:HCS}. 
Efficient implementations of feature extractors and a study of feature importance on datasets which represent the state-of-the-art are subject to future work. 

In order to facilitate portfolio analysis for evaluation purposes and for ad-hoc portfolio generation, we want to integrate our exact portfolio-search approach into the Python package~\emph{gbd-tools}, so it can be directly used in queries to the database GBD~\cite{iser2020collaborative}. 
