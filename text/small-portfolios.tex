...
\todo{still inconsistent/mixed use of terms "small portfolio", "$k$-portfolio" and "$k$-Portfolio"}
...
\todo{need to re-add some description/definition what a portfolio is}
...
For approximate determination of optimal portfolios if size $k$, we apply \emph{beam search} with varying beam widths. 
...
\todo{runtimes are continuous in our dataset}
...
That score yields a trade-off between pace and the number of solved instances. 
\todo{which pace? do you mean runtime of the solvers?}
...
$$
\todo{might add m as another parameter or superscript to the modified cost function}
...
The simplest way to solve the $k$-Portfolio Problem is by exhaustively searching all portfolios with $k$ solvers. 
...
We analyze the $k$-Portfolio Problem for three definitions of the cost function in the optimization objective.
...
In \emph{random search}, we average over the repetitions of random sampling.
The displayed \emph{upper bound} is calculated based on the runtimes of the worst performing $k$-portfolio. 
\todo{that's wrong}
...
\todo{add link to upper bound equation}
...
\begin{figure}[t]
	...
	\caption{
		Test-set MCC for random-search portfolios, using random forests with 100 trees.
