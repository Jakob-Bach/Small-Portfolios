...
It has addressed the essential question of how large portfolios which are drawn from that set of solvers should be.
...
In order to facilitate portfolio analysis for evaluation purposes and for ad-hoc portfolio generation, we integrated our exact portfolio-search approach into the Python package~\emph{gbd-tools}~\cite{iser2020collaborative}. 
...
Efficient implementations of new feature extractors and studying feature importance on datasets which represent the state-of-the-art are subject to future work. 
...
This general trend has been mitigated in the competitions considered in our study by also highlighting SAT solvers that perform particularly well on specific instance families~\cite{SC2020:AIJ} or on a (not further specified) large subset of the benchmark instances (cf.~Example~\ref{ex:specialprice}).
Such additional evaluations provide reference points for a more informed portfolio composition, and can even positively impact research on stable solvers~\cite{Fleury:2021:AllUip}.
For future analyses, we plan to increase the amount of specialized solvers and configurations targeting \emph{well-defined} subsets of benchmark instances.
