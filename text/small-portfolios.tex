...
%Solver portfolios can also be used in combination with runtime prediction models for instance-specific algorithm selection. %KB: Wieso brauche ich dafür Portfolios? Diese Art Vorhersage kann ich vermutlich mit kleinem Aufwand für viele (fast alle) Solver machen, oder nicht? Und dann weiter: Könnte ich nicht, basierend auf diesen Vorhersagen, 'on the fly' ein Portfolio für ein bestimmtes Problem zusammenstellen?
...
%KB: Hier sollte die Reihenfolge der Aussagen vertauscht sein. Zuerst das hier sagen, dann das, was gemacht wird.
...
For the evaluation of our approach, for all $k$, we consider both the theoretically optimal performance of portfolios %KB: Was soll das 'theoretically' hier?
...
We rely on two baselines, one that randomly samples $k$ solvers, and and another one that selects the $k$ fastest individual solvers. %KB: Die zweite Baseline habe ich hier nicht verstanden.
...
One systematic analysis of portfolios with varying size~$k$, which has some simliarities to our study, is conducted by Adamini et al.~\cite{amadini2014empirical, amadini2016extensive}. 
...
They evaluate their approach on very easy instances of an allocation problem which are already solved within a timeout of 0.1~s.%MI: solvers?

While we build on the theoretical results of Nof and Strichman, our evaluation is different.
In order to deterimine $k$-Portfolios, we evaluate a \emph{beam search} algorithm with varying widths, random portfolios of size $k$ as a baseline, and for optimal solutions we give an encoding as an integer optimization problem. 
...
We not only evaluate the theoretical lower bound of runtimes of $k$-portfolios drawn from these datasets with varying size $k$ w.r.t. optimal and approximate solutions to the $K$-Portfolio Problem. 
...
For the $316$ instances of the SAT Competition~2020, the three overall fastest solvers `win' on only 46, 38, and 26 instances, respectively.
For the $325$ instances of the SAT Competition~2021, the three overall fastest solvers `win' on only 25, 22, and 20 instances, respectively.
...
For the SAT Competition~2020, the optimal $1$-portfolio has a penalized runtime $5.51$ times as high as the $48$-portfolio.
This ratio reduces to $1.89$ for the best $5$-portfolio and $1.25$ for the best $10$-portfolio.
For the SAT Competition~2021, the respective ratios are $4.74$ for $k=1$, $1.58$ for $k=5$ and $1.17$ for $k=10$.
...
For example, the best $2$-portfolio found by \emph{greedy search} has a $14.7\%$ higher cost than the \emph{optimal solution} for the SAT Competition~2020.
...
The performance of \emph{k-best} relative to the other approaches also differs between the SAT Competitions~2020 and 2021.
In 2020, \emph{k-best} is closer to the optimal solution, while in 2021, \emph{k-best} is closer to the expected value of \emph{random sampling}.
This indicates that building a portfolio of complementary solvers is more important in 2021, rather than picking just the best individual solvers.
...
\begin{figure}[t]
	...
	\caption{
		Test-set portfolio performance for different $k$ and search algorithms.
		Data from the SAT Competition 2020.
	}
...
As a notable exception, \emph{beam search}, \emph{k-best}, and the \emph{optimal solution} show similar performance on the test data of the SAT Competition~2020.
...
For the test data of the SAT Competition~2021, which we do not display here, \emph{k-best} still shows clear difference to \emph{beam search} and the \emph{optimal solution}.
...
For example, the optimal $2$-portfolios do not contain the optimal $1$-portfolios for both SAT Competitions.
...
For $k=5$, all correlations are in $[-0.40,0.17]$ for data from the SAT Competition~202, and $[-0.24,0.23]$ for data from the SAT Competition~2021.
...
This effect is stronger for data from the SAT Competition~2021.
...
\begin{figure}[t]
	...
	\caption{
		Portfolio performance for \emph{optimal solution} portfolios, comparing the portfolio's virtual best solver (VBS), single best solver (SBS), and score with prediction model.
		Data from the SAT Competition 2020.
		Prediction model: random forests with 100 trees.
		The performance of the the global SBS, i.e., not only considering solvers from the portfolio, is shown as horizontal line.
	}
