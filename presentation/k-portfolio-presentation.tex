...
\begin{frame}[t]{Motivation}
	\begin{itemize}
		%JB: ongoing research on new solvers / modification of existing solvers, evaluated on diverse benchmark instances
		...
		%JB: in fact, Main Track of SCs favors general-purpose solvers; results might be even more pronounced for scenarios with more specialized instances and solvers
		\begin{itemize}
			...
			\begin{tabular}{ccc}
				...
				\bottomrule
			\end{tabular}
			%JB: note that even if solver did not win, it might be very close to winner; however, we'll see later that complementarity extends to runtimes / PAR-2 scores as well
		\end{itemize}
		\vspace{\baselineskip}
		\pause
		...
		\begin{itemize}
			...
			%JB: though Place 2 for Main Track SAT
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[t]{Problem Definition}
	...
	\begin{itemize}
		...
		\end{itemize}
		%JB: overall, our paper is an empirical study of such k-portfolios, comparing impact of k, solution approaches, etc. (details will follow)
	\end{itemize}
\end{frame}
...
\begin{frame}[t]{Results -- Portfolio Search (VBS on Training Set)}
	\begin{figure}[htb]
		...
		%JB: all approaches: strong decrease with k; optimal 10-portfolio only 25% (SC2020) / 17% (SC2021) higher cost than portfolio out of all solvers, while single best solver has 451% (SC2020) / 374% (SC2021) higher cost (i.e., 5.51 / 4.74 times the cost); figures less impressive if timeout instances kept (see Froleys at al. (2021)), because these strongly impact overall score (add ~2000 s to each portfolio)
