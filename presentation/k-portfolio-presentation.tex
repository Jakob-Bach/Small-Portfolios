...
\begin{frame}[t]{Related Work}
	\begin{itemize}
		...
		%JB: Amadini et al.: one search heuristic ("local search"), no exact solution
		%JB: Amadini et al.: compare various classifiers and multiple sophisticated portfolios approaches (like SATzilla)
		%JB: work of Dang is brand-new, presented at ModRef workshop two days ago
		%JB: Dang: only exact solution for varying k, no prediction model, but analysis of marginal contribution
...
\begin{frame}[t]{Solution Approaches -- Integer Linear Program}
	...
	\begin{alignat*}{4}
		\onslide<3->{
			...
			%JB: average over instances and use of cost c(i,s) should be intutive, variable x_{i,s} is new
		}
		\onslide<2->{
