...
\begin{frame}[t]{Motivation}
	\begin{itemize}
		%JB: why use portfolio: shouldn't I use just one solver for each instance? short answer: no; long answer:
		...
		%JB: no best solver for all instances; complementary: some solvers good on some instances, others good on other instances; to give concrete example:
		\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				%background: besides submodularity, also monotonicity (i.e., adding solvers to portfolio does not make costs worse) and non-negativity required
			\end{itemize}
			\pause
